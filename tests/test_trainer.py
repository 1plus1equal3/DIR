...
def test_trainer():
    ...
    config.NUM_EPOCHS = 3  # Only train 3 epochs
    config.LEARNING_RATE = 1e-3
    config.LOG_EVERY_N_STEPS = 5
    config.SAVE_EVERY_N_EPOCHS = 2
    config.VAL_EVERY_N_EPOCHS = 1
    config.VISUALIZE_EVERY_N_EPOCHS = 1
    config.NUM_VIS_SAMPLES = 2
    config.CHECKPOINT_DIR = "./test_checkpoints"
    config.MODEL_NAME = "tiny_test_model"
    config.WANDB_PROJECT = "test_rectify"
    ...
    try:
        trainer = create_trainer(
            ...
            wandb_api_key_path=config.WANDB_API_KEY_PATH,
